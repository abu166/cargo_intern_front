...
import { Package, Train, CheckCircle, Clock, MapPin, RefreshCw } from 'lucide-react';
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Погрузка и разгрузка вагонов • Станция: <span className="font-medium">{user?.station || 'Не указана'}</span>
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className={`text-center py-12 rounded-lg border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            На вашей станции пока нет отправлений
          </p>
        </div>
      ) : (
          ...
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className={`rounded-lg border overflow-hidden ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                {isComplete && (
                  <div className={`border-t p-3 ${isDark
